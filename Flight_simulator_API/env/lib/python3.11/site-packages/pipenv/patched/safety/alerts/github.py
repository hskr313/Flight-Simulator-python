...
def github_pr(obj, repo, token, base_url):
    ...
    branch_prefix = (
        obj.policy.get("alert", {})
        .get("security", {})
        .get("github-pr", {})
        .get("branch-prefix", "pyup/")
    )
    pr_prefix = (
        obj.policy.get("alert", {})
        .get("security", {})
    ...
    label_severity = (
        ...
        .get("security", {})
        .get("github-pr", {})
        .get("label-severity", True)
    )
    ignore_cvss_severity_below = (
        obj.policy.get("alert", {})
        .get("security", {})
        .get("github-pr", {})
        .get("ignore-cvss-severity-below", 0)
    )
    ignore_cvss_unknown_severity = (
        obj.policy.get("alert", {})
        .get("security", {})
        .get("github-pr", {})
        .get("ignore-cvss-unknown-severity", False)
    )
    ...
    for name, contents in obj.requirements_files.items():
        ...
        for pkg, remediation in obj.report["remediations"].items():
            ...
            vulns = [
                ...
                if x["package_name"] == pkg
                and x["analyzed_version"] == remediation["current_version"]
            ]
