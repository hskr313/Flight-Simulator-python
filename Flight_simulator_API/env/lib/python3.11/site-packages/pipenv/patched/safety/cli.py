...
from pipenv.patched.safety.safety import (
    get_packages,
...
@click.option(
    ...
    mutually_exclusive=["telemetry", "disable-telemetry"],
    is_flag=True,
    show_default=True,
)
...
@click.option(
    ...
    with_values={"output": ["screen", "text", "bare", "json"], "bare": [True, False]},
    callback=json_alias,
    hidden=True,
    is_flag=True,
    show_default=True,
...
def check(
    ...
    try:
        ...
        params = {
            ...
            "project": project,
            "audit_and_monitor": server_audit_and_monitor and audit_and_monitor,
        }
        ...
        if save_json or (server_audit_and_monitor and audit_and_monitor):
            ...
            json_report = SafetyFormatter(output="json").render_vulnerabilities(
            ...
            if server_audit_and_monitor and audit_and_monitor:
                ...
                r = post_results(
                    key=key,
                    proxy=proxy_dictionary,
                    safety_json=json_report,
                    policy_file=policy_contents,
                )
        ...
        if stderr_announcements and (
            ...
            click.secho(
                SafetyFormatter(output="text").render_announcements(
                    stderr_announcements
                ),
                fg="red",
                file=sys.stderr,
            )
        ...
        click.secho(
            output_report, nl=should_add_nl(output, found_vulns), file=sys.stdout
        )
...
def review(ctx, full_report, output, file):
    ...
    announcements = safety.get_announcements(
        key=None, proxy=None, telemetry=ctx.parent.telemetry
    )
    output_report = SafetyFormatter(output=output).render_vulnerabilities(
        announcements, vulns, remediations, full_report, packages
    )
...
def generate(ctx, name, path):
    ...
    try:
        ...
        click.secho(
            f"Unable to generate {name}, because: {str(exc)} error.",
            fg="red",
            file=sys.stderr,
        )
        sys.exit(EXIT_CODE_FAILURE)
...
def validate(ctx, name, path):
    ...
    click.secho(
        f"The Safety policy file was successfully parsed with the following values:",
        fg="green",
    )
