            ...
            while True:
                if len(stack) > 0:
                    ...
                    cldn = [
                        ...
                        if not any(fnmatch.fnmatch(c.key, e) for e in exclude)
                    ]
                    m[n] = cldn
                    seen.add(n.key)
                    for c in cldn:
...
def render_cycles_text(cycles):
    if cycles:
        ...
        for a, b, c in cycles:
            print(
                f"* {a.project_name} => {b.project_name} => {c.project_name}",
                file=sys.stderr,
            )
...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Display dependency tree as a Mermaid graph. "
            "This option overrides all other options."
        ),
    )
    parser.add_argument(
...
def handle_non_host_target(args):
    ...
    if of_python != os.path.abspath(sys.executable):
        ...
        if args.output_format:
            print(
                "graphviz functionality is not supported when querying"
                " non-host python",
                file=sys.stderr,
            )
            raise SystemExit(1)
...
def get_installed_distributions(local_only=False, user_only=False):
    try:
        ...
        return misc.get_installed_distributions(
            local_only=local_only, user_only=user_only
        )
    else:
