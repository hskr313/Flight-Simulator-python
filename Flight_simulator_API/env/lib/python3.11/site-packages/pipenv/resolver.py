...
class Entry:
    ...
    def get_markers_from_dict(cls, entry_dict):
        ...
        markers = {
            normalize_marker_str(f"{k} {entry_dict.pop(k)}") for k in keys_in_dict
        }
    ...
    def pipfile_name(self):
        return self.project.get_package_name_in_pipfile(
            self.name, category=self.category
        )
    ...
    def validate_specifiers(self):
        if self.is_in_pipfile and not self.pipfile_entry.editable:
            return self.pipfile_entry.requirement.specifier.contains(
    ...
    def get_parent_deps(self, unnest=False):
        ...
        parents = []
        for spec in self.reverse_deps.get(self.normalized_name, {}).get(
            "parents", set()
        ):
    ...
    def get_constraints(self):
        ...
        constraints = {
            c
            for c in self.resolver.parsed_constraints
